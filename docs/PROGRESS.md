...
- **Alpha 4.20.3 — Emissive de-saturation pass** — playtest verdict on 4.20.2: "All of the buildings are just this ugly light brown colour. the entire building." Math check: at `emissiveIntensity 0.32` the warm-cream `0xffd8a8` was adding `(81, 69, 54)` RGB to every pixel of every face — overwhelming the underlying zone diffuse colours and reducing every building to "cream + a hint of whatever". Lambert's `.emissive` is a uniform additive term, not a multiplicative one, so even modest values dominate when the diffuse light is dim (which is exactly what happens at night).
  - **`emissiveIntensity` capped at 0.05** (was 0.32). Now adds only `(13, 11, 8)` RGB at peak — a subliminal warm floor that prevents pitch-black faces without flattening palettes. Red brick still reads red, blue glass still reads blue.
  - **Moon intensity bumped 0.55 → 0.75** to compensate. The visible "buildings are lit" feeling now comes from directional moonlight on each face (which respects the underlying diffuse colour) plus the existing `litWindowsMesh`. Emissive is just a thin baseline that prevents pure-black silhouettes.
  - **Moon tint slightly de-saturated** (`0xa8b8e8` → `0xc4d0e8`) so the cool blue cast doesn't tint everything blue at the same time emissive tries to tint everything cream.
  - No save schema change. Bundle 953 KB raw / 251 KB gzipped (unchanged).

- **Alpha 4.20.2 — Light ON the buildings (not on the ground): moonlight + emissive** — playtest verdict on 4.20.1: "i dont want ground light I want lighting on the buildings, you know to make it feel like there is some depth. The lighting in this style is very flat." Right call — bumping the ground halo was solving the wrong problem. Real cities at night are vibrant because the BUILDINGS themselves are lit (interior light spilling through windows, faces lit from streetlamps below) AND because there's still directional shading from the moon giving face-to-face contrast. The original 4.20 had no moon and no per-mesh emissive, so building faces went uniformly dim and flat.
  - **New `moonLight` `DirectionalLight`** (cool blue-white `0xa8b8e8`). Position is mirrored to the sun's azimuth in `applyTimeOfDay` — when the sun dips below the horizon, the moon casts from above-and-opposite. Intensity ramps 0 → 0.55 with night phase. Restores the directional shading contrast on building faces that the dim sunLight loses at night (sunLight stays at 0.18 intensity at midnight by design — but now there's a second, opposing source painting the OTHER side of every face, so depth comes back).
  - **Per-mesh emissive on the three building meshes** — `buildingsMesh` (zoned R/C/I/MU), `skyscrapersMesh`, and the `cityBuildingsGroup` mesh (services / landmarks / Mayor's Mansion / capitals). Material gains `emissive: 0xffd8a8` (warm cream tint) + `emissiveIntensity: 0` at construction; `applyTimeOfDay` ramps the intensity to **0.32** at deep night across all three meshes via a single `buildingEmissive` value. Buildings now appear visibly lit-from-within at night without their underlying zone colours getting washed out (emissive adds, doesn't multiply, so red brick stays red and blue glass stays blue — they just glow softly).
  - **Ground halo glow gated to opacity 0** — kept the `buildingGlowMesh` build code intact (it's cheap, one quad per L2+ tile) but `applyTimeOfDay` now sets opacity 0 + visible false on it permanently. Reversible if a future visual ever wants ground spill, but for now the user explicitly doesn't want it.
  - **Net effect**: dense L3/L4 blocks + skyscrapers visibly glow at night with directional shading on every face. The flatness is gone. Streetlamps + park lights still read as the brightest accent layer; building emissive is the "ambient interior glow" layer underneath.
  - No save schema change. Bundle 953 KB raw / 251 KB gzipped (unchanged — pure material + light tweaks).
