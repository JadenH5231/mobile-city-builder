...
- **Beta 1.6.18 — Bridges light up at night + supply-chain stockout tells the player why** — two user reports bundled because each is small. (1) "Bridges don't light up and it makes highways look a little funny because they do." Avenue lamps (`buildNightLightsMesh`) and lamp glows (`buildLampGlowMesh`) both had a `!t.bridge` filter that explicitly skipped bridge tiles, so an avenue crossing water went dark for one segment while the rest stayed warmly lit. Lane stripes had the same skip (`if (ta?.bridge || tb?.bridge) skip`) on the theory that "ramp stripes would float in mid-air" — but `yStripeA/yStripeB` already track per-endpoint y values (which handle bridge lifts), so a stripe spanning a land-tile endpoint and a bridge-tile endpoint naturally follows the ramp surface. Fix: drop the `!t.bridge` filter on both lamp paths (use `BRIDGE_LIFT` for bridge tiles so lamps sit on the deck) and drop the stripe skip entirely. A highway over water now keeps its white shoulder + double-yellow median across the bridge segment; an avenue over water gets its sidewalk lamps and warm glow pools. (2) "The supply chain system is also still completely broken. All stores I've clicked on say 0% supply." Investigation: the supply chain *is* working — fresh tiles start at supplies=1, consumption drains them 0.10-0.18 per month, deliveries refill them. But cities without industry, warehouses, OR an outside-world road connection have no truck supply at all, so stores legitimately drain to 0% and stay there. The bug was that the panel showed `📦 Supplies 0%` with zero explanation, so the user couldn't tell whether it was a code bug or a configuration issue. Fix: added a `block`-toned diagnostic reason in `diagnoseTile` for commercial / mixed / big_box tiles at ≤5% supplies — "Out of stock — earns no revenue. Build factories, warehouses, or extend a road to the map edge for imports." Plus a `warn` at <30% — "Low stock — waiting on a delivery truck" — so the player sees the chain working before it bottoms out. The simulation itself is unchanged; this is purely a visibility-of-state fix so the player understands what's happening.

- **Beta 1.6.17 — Lit windows snap to each variant's actual body face instead of floating off / hiding inside** — user feedback: "Some of the lighting looks like it's floating off the building, I see it in medium density but check others to be sure." Root cause: when the alpha lit-windows code was written (Beta 1.6.8), every developed tile rendered as a uniformly-sized box, so a hardcoded `halfW = 0.30` worked. The current building system (`BuildingVariants.ts`) emits variant-specific bodies whose `w` and `d` range ~0.45..0.85 tile units and rotate one of four cardinal yaws, plus the variant centre is jittered ±0.025 by a deterministic hash. The lit-windows pass kept using `cx = t.x + 0.5`, `cz = t.y + 0.5`, and `halfW = 0.30` — so on small variants (L2 with body.w=0.55) the +Z/+X windows floated ~0.025 off the wall, on large variants (L3 with body.w=0.80) they hid ~0.10 inside the body (depth-occluded by the opaque face), and the jitter offset every building so the four faces never lined up evenly. **Fix:** new `getVariantBodyFootprint(zone, density, tileX, tileY)` exported from `BuildingVariants.ts` that re-runs the *exact same* variant pick / jitter / yaw resolution `buildVariantParts` uses, then returns `{ cx, cz, halfX, halfZ, height }` in world space (with the body's w/d swapped if the yaw is 90°/270°). Renderer's lit-windows pass for L2+ residential / commercial / mixed / industrial now uses those values to place windows on the actual body face, with the column spread scaled to each face's actual half-width (`halfX * 0.7` for ±Z faces, `halfZ * 0.7` for ±X faces). L3+ apex beacons also re-centred on the variant centre instead of the tile centre. Verified at close zoom: windows sit flush against the body faces on all four sides for every density × zone, with no floating gap and no occluded-inside variants. Skyscraper, luxury, and L1 paths unchanged (skyscrapers use their own anchor-based footprint logic; L1/luxury have no lit windows).

- **Beta 1.6.16 — Road tier upgrades show up immediately instead of waiting for the next unrelated placement** — user feedback: "The roads don't visually update very well, is there a way to optimize it? Sometimes it requires I place something before it updates on the screen. It makes it hard to know what road has or hasn't been upgraded." Root cause in `src/world/Grid.ts`: `setRoadEdge(on=true)` always calls `setRoad` on both endpoints (which mutates `roadType` — the upgrade itself works), but then short-circuits to `return false` if the edge bit already existed. The caller in `Game.applyRoadStroke` keys the `roadsChanged` flag off that return value (`if (this.grid.setRoadEdge(...)) roadsChanged = true`), so a tier-only upgrade on an already-connected pair never triggered the mesh rebuild — the player saw the upgrade only when some later unrelated placement happened to set `roadsChanged = true`. Fix: capture each endpoint's `setRoad` return value (which already correctly reports "changed" on tier flip) and propagate it up. `return aChanged || bChanged` when the edge already existed; `return true` when the edge itself is new. Mirrored the same fix in `setBridgeRoadEdge` so bridge tier upgrades behave identically. Verified by painting a local road, then upgrading the middle segments to avenue — the avenue-width body with double-yellow stripes appears immediately, with clean half-edge tapers to the unchanged local segments on either end.

- **Beta 1.6.15 — Floating Rotate button no longer covers the tool-cost pill during big-build placement** — user screenshot showed the orange "↻ ROTATE" pill stamped directly across the green "City Hall · $1,500,000" cost pill, hiding the price tag the player needed to see before tapping. Both elements pin themselves to the bottom centre of the viewport (the only horizontal positioning they have is `left: 50%; transform: translateX(-50%)`), and their bottom offsets were nearly the same: cost pill at `safe-area-inset-bottom + 72px` (~34px tall, top edge at ~106px), Rotate button at `+ 84px` — only a 12px gap between the pill's top and the Rotate's bottom, so on every paid big-build placement the Rotate covered the cost. Fix: bump `.rotate-monument-btn { bottom: ... + 120px }` in `src/styles.css` so the Rotate sits clearly above the cost pill (~16-24px gap measured at desktop and iPhone mini). Verified at 375x812: stack reads ROTATE → cost pill → toolbar with both elements fully legible. No JS changes; pure CSS positioning fix.

- **Beta 1.6.14 — Avenue/highway no longer bleeds into adjacent local roads at junctions** — user (Toorko) in #bug-reports: "Is this expected? I put an avenue only on the diagonal, but I'm not sure why some roads that aren't avenues changed as well." Screenshot showed local horizontal roads with avenue-width stubs at every point they met the diagonal avenue. Root cause in `src/engine/Renderer.ts` `buildRoadMesh`: each cross-tile edge rendered as a single quad with a single tier — initial implementation picked **max** tier (avenue's width pushed onto the local); flipping to **min** just reversed the artifact (the avenue narrowed to local at junctions). User pointed out the correct mental model: *neither* road should change width where they meet — each tile's footprint stays at its own tier. **Fix: split every edge into two half-quads**, one per endpoint, each rendered at that endpoint's own tile's tier width and colour, meeting at the tile boundary. A same-tier edge produces two identical halves that visually merge into one continuous segment. A cross-tier edge produces a clean width step at the boundary: avenue stays avenue-width up to the local's tile edge, local stays local-width from there on. Refactored the inline edge-loop body to use a `emitHalf(sx, sz, ex, ez, ys, ye, px, pz, hex)` helper and doubled `totalQuads` allocation. Lane stripes: kept the existing center-to-center pattern for same-tier edges; skipped on cross-tier edges (a half-length avenue double-yellow or two local dashes would read as a stub, and the width step itself is enough of a visual cue). Bridges keep the simpler min-tier behaviour from the first iteration — cross-tier bridge junctions are rare, and splitting the deck/rails/pillars properly is a bigger refactor that can wait. Verified with three scenarios: (a) vertical avenue + 3 horizontal local branches → avenue stays wide along its entire length, locals stay narrow up to the boundary; (b) all-local cross grid → identical to pre-1.6.14 (same-tier halves merge seamlessly, dashed centerlines preserved); (c) NW-SE diagonal avenue + 3 horizontal local branches matching the user's exact screenshot → neither road changes width at the three T-junctions.

- **Beta 1.6.13 — Lit windows visible from every camera yaw + brighter skyscrapers** — user feedback: "I need the lights in the windows of the higher density buildings to be seen from all directions. Also, skyscrapers need more lighting, also commercial buildings should have some window lights, it can be less than residential but shouldn't be none." Diagnosed the root cause of Beta 1.6.12's incomplete fix: `buildLitWindowsMesh`'s `addWindow` helper produces quads with a fixed front-face normal regardless of which face they're placed against (X-quads always face -z, Z-quads always face +x), and the lit-windows `MeshBasicMaterial` defaulted to `FrontSide`. So even though 1.6.12 added a window on all four faces, two of every four were back-face-culled — half the city looked dark whenever the camera rotated 180° via the Alpha 4.7 yaw control. Three edits in `src/engine/Renderer.ts`:
  - **DoubleSide on the lit-windows material.** One-line fix: `side: DoubleSide` on the `MeshBasicMaterial` at the end of `buildLitWindowsMesh`. The building body is still opaque and still depth-writes, so the far-side window (the one behind the building from the camera's perspective) is still depth-occluded — DoubleSide doesn't add overdraw, it just lets the previously-culled +Z and -X face windows render when the camera looks AT those faces. This single change is what makes commercial L2+ visibly "have window lights" from camera angles where they used to look dead — the windows were always being emitted, just culled.
  - **Skyscraper main tower: ~50% → ~75% lit.** Changed the dim pattern from `(... & 1) === 0` to `(... & 3) === 3` (skip only when both low bits set). Matches residential L2+'s density. The skyscraper is the iconic skyline silhouette, so it should glitter, not just barely flicker — and the DoubleSide fix means the brighter pattern wraps every face uniformly.
  - **Skyscraper second tower (twin designs): same ~75% pattern.** Kept the second tower visually consistent with the main shaft so twin designs don't look lopsided.
  - **Commercial L2+ left unchanged** at its existing ~50% lit ratio (already less than residential's ~75%, which satisfies "less than residential but shouldn't be none"). The user's complaint here was about visibility, not absolute brightness — the DoubleSide fix resolves the "shouldn't be none" half by making the previously-invisible faces show their windows. Industrial stays at its sparse ~25% security-floodlight pattern.
  - **Verified** at all four cardinal yaw angles (45° / 135° / 225° / 315°): the test skyscraper shows the dense ~75% pattern on both visible faces at every angle, and adjacent L2/L3 commercial / industrial / mixed-use buildings show their windows uniformly.

- **Beta 1.6.5 — Three bundled fixes (Discord revert + supply chain forgiveness + warehouse parking)** — three user-asked changes shipped together since they're each small.
  - **1.6.3 — Discord revert.** User: "Canada isn't eligible for monetization, please reverse any discord connection." Uninstalled `@discord/embedded-app-sdk`, deleted `src/discord/`, removed Discord init + import from `src/main.ts`, removed `VITE_DISCORD_CLIENT_ID` from `.github/workflows/deploy.yml`, deleted `docs/DISCORD_SETUP.md`. Codebase has zero Discord traces. The `VITE_DISCORD_CLIENT_ID` GitHub secret (if set) becomes inert since no build code reads it; can be deleted in the GitHub UI when convenient.
  - **1.6.4 — Forgiving supply chain.** User wanted commercial tiles to start with supplies (they already did per `Tile.supplies = 1` default), drain at differing random times (so not every store empties together), and make a PO before running out (so they have a chance to be restocked). Two changes in `src/simulation/SupplyChain.ts`:
    - **Per-tile consumption jitter.** Pre-1.6.4 fixed `0.18/month` rate → every tile hit 0 simultaneously, hard revenue cliff. Now `consumptionRate(x, y) = 0.10 + 0.08 × hash(x, y)` in [0.10, 0.18], deterministic per tile so it's stable across save/load. Stores trickle empty over months 6-12 instead of all on month 5.
    - **Purchase-order priority.** New `pickRestockNeedingCommercialTile(grid)` does weighted-reservoir sampling over commercial tiles below `RESTOCK_REQUEST_THRESHOLD = 0.55`, lower-supplies = higher pick weight. `Vehicles.attemptTruckSpawn` consults this BEFORE the random destination roll, so a 50% store gets a delivery proactively. Stores making POs at half-full have a real chance to be restocked before zero, instead of relying on random chance.
  - **1.6.5 — Warehouse parking fills up.** User: "no one uses the parking lots near warehouses, please fix." Warehouses had no commute traffic — only curbside-delivery trucks visited. Added `pickRandomBuildingTile(grid, 'warehouse')` as a 6% branch in `Vehicles.attemptSpawn` resident-destination roll (mirroring the existing forestry/farm employment branches). The 1.4.2 `findStallNearDest` auto-detects warehouse-adjacent parking and reserves stalls. Resident-spawn distribution rebalanced: 42% C, 28% I, 16% forestry, 8% farm, **6% warehouse**.
  - **SW cache `mq-city-v18` → `mq-city-v19`.** Save schema unchanged. Bundle size −20KB (Discord SDK removed).

- **Beta 1.6.2 — Discord Activities integration (code scaffold)** — user feedback: "I would like to host this game as an app on discord. How do I do that? … you do all of the things that you can do and tell me what's left that i have to do and how to do it." Implemented the entire code-side: SDK install (`@discord/embedded-app-sdk` v2.5), isolated `src/discord/DiscordContext.ts` module with `isDiscordActivity()` runtime check + async `initDiscord()` SDK handshake + `getDiscordContext()` accessor, `src/main.ts` kicks off init after auth and seeds default city name from the Discord display name on fresh saves, `.github/workflows/deploy.yml` passes a new `VITE_DISCORD_CLIENT_ID` secret to the Vite build (same pattern as `VITE_SUPABASE_URL`). Discord path is **fully opt-in** — when the env var is unset (default for the public mqcity.app deploy), the integration is byte-equivalent to pre-1.6.2. Wrote `docs/DISCORD_SETUP.md` with the 6 manual steps the user needs to do in the Discord Developer Portal (create app → enable Activities → add URL Mappings → add GitHub secret → submit for Shelf review → test). Verified dist/index.html ships with ZERO inline scripts/styles so Discord's strict CSP doesn't block the build. SW cache `mq-city-v17` → `mq-city-v18`. Bundle +~50 KB raw / +~15 KB gzipped (one-time Discord SDK cost).

- **Beta 1.6.1 — "Clear visual ghosts" debug button in Settings** — user feedback: "Is there a way to add a debug button that removes weird visual artifacts? Sometimes bridges don't delete properly and it's annoying. I think in some cases the actual entity is gone, but it's the visual that's sticking around." Added a new Diagnostics section to the Settings panel with a single button that triggers a full renderer-mesh rebuild via `Renderer.refreshTheme(grid, mood, months, forestry, farm)` — the same code path the Theme picker uses for in-place repaints. Drops every cached world mesh (terrain, zones, paths, roads INCLUDING bridges + bridge supports, road ornaments, buildings, services, beautification) and re-derives them from authoritative Tile state. Any orphan mesh whose tile state has been cleared gets purged. Pure wiring: new HTML button + optional `onClearGhosts` hook on `bindSettingsPanel` + a passthrough in main.ts. No new renderer code (the rebuild path already existed for theme swaps). Settings panel stays open after the click (unlike Show Tutorial) so the player can re-click if needed. Status-toast confirmation "Visual artifacts cleared" fires on success. SW cache `mq-city-v16` → `mq-city-v17`.

- **Beta 1.6 — Warehouses + supply chain (commercial needs supplies to earn)** — user feedback: "I want a new industry, warehouses. These buildings receive goods from industry and ship it to commercial, adding one new step to the supply chain. I want you to do some heavy lifting making a new supply chain system where commercial buildings need supplies to produce their tax revenue, if they don't get them or run out before a new shipment arrives they stop producing tax revenue. Outside connections can also bring in goods if there isn't enough industry but this should come at a slight financial penalty as well. Warehouses should work in any direction, be truly modular and also require parking lots for employees to be operational. If a map doesn't have warehouses they can ship from industry directly, but make warehouses a more efficient way of doing it." Substantial feature spanning 6 phases.
  - **New `Tile.supplies: number` (0..1, default 1)** + `Tile.importSource: boolean`. Persists across save load. Save schema v28 → v29 (pre-29 saves load with supplies=1 so long-running cities aren't suddenly choked).
  - **New `src/simulation/SupplyChain.ts` module.** Owns: `tickMonth(grid)` — monthly consumption (commercial 0.18, warehouse 0.08); `deliver(grid, x, y, source)` — truck-arrival callback that refills the destination based on the source kind (industry-direct / warehouse / import / industry-to-warehouse); `commercialSupplyState(grid)` — query the Economy reads each month for the supply-aware revenue multiplier; `isCommercialConsumer(t)` — predicate for "this tile needs supplies."
  - **Economy.runMonth folds a `supplyMultiplier` into the commercial revenue line.** A tile at 0 supplies contributes 0 revenue; an import-sourced tile applies the `IMPORT_REVENUE_MULTIPLIER = 0.75` penalty (the user's "slight financial penalty"). Cached on `Economy.lastSupplyMultiplier` / `lastImportedFraction` for future UI.
  - **New `warehouse` building type** in `types.ts` Building union. `place_warehouse` Tool added to the Industry toolbar group. Cost $1500/tile, upkeep $80/tile, Metro-tier milestone unlock. 10 faction-specific stance values added in `Council.ts` — Chamber +0.8 (loves efficient logistics), Working Families +0.6 (jobs!), Drivers +0.5, Taxpayers +0.4, YIMBYs −0.2, Safer Streets −0.3, NIMBYs / Transit −0.4, Greenleaf −0.5, Hometown −0.6.
  - **`Renderer.warehouseClusterParts()`** — fully modular per-tile emission mirroring the 1.4.1 bigBoxClusterParts pattern. Industrial-freight palette (greys + safety yellow accents), distinct from the retail palette of big_box. Per-tile loading-dock doors (3 per S-exterior tile, the warehouse signature), rooftop vents + skylight strips, parapet brand stripe, dark concrete dock band. Inner-corner filler for L/T/U shapes. Absorbs adjacent parking_lot tiles into the apron like big_box.
  - **`Vehicles.attemptTruckSpawn` warehouse-aware.** 40% I→W (restocks warehouse buffer), 40% W→C (most efficient delivery), 20% I→C direct (fallback when warehouses aren't around). Each branch falls through to I→C if its prerequisite is missing, so a city with no warehouses still works (less efficiently). Truck carries `truckSource: DeliverySource` so the arrival callback knows what to refill.
  - **`Vehicles.spawnImportTruck` + `Game.tickImportTrucks`** — outside-connection fallback. Throttled at ~1 attempt per 4 sim sec. When city is connected AND a commercial tile is below 30% supplies, spawn an import truck from a random edge road. Marks destination `importSource = true` (cleared on next domestic delivery).
  - **Place-time validation extended** to `warehouse` (same flat-dry-land + no-zoning rules as big_box / parking_lot).
  - **TileInfoPanel chip** — `📦 Supplies NN%` on commercial / warehouse tiles, colour-tiered (block <5%, warn <30%, info <60%, good ≥60%). Extra `🌐 Import-sourced (−25%)` chip when penalty active.
  - **SW cache `mq-city-v15` → `mq-city-v16`.** Clean typecheck + build.
  - **Design principle for the next "consumer-type" tile to plug into the supply chain (residential needs water? civic needs power?):** add the `supplies`-equivalent field on Tile, decrement it in SupplyChain.tickMonth, gate the relevant revenue/development line in the simulation. Don't fragment inventory state across modules — SupplyChain owns the math.
